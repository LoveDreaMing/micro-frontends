 ...
 */
const naughtySafari = typeof document.all === "function" && typeof document.all === "undefined"; // safari 特殊判断（document.all 的怪异 typeof 行为）
...
export const isCallable = (fn: any) => {
  ...
  const callable = naughtySafari ? typeof fn === "function" && typeof fn !== "undefined" : typeof fn === "function"; // 在 safari 环境做特殊判断
  if (callable) {
...
export function isBoundedFunction(fn: CallableFunction) {
  ...
  const bounded = fn.name.indexOf("bound ") === 0 && !fn.hasOwnProperty("prototype"); // 判断函数名以 "bound " 开头且无 prototype 属性 => 被 bind 过
...
export function isConstructable(fn: () => any | FunctionConstructor) {
  const hasPrototypeMethods =
    fn.prototype && fn.prototype.constructor === fn && Object.getOwnPropertyNames(fn.prototype).length > 1; // 判断 prototype 上是否有除 constructor 以外的方法（表明可作为构造函数）
  ...
  if (!constructable) {
    ...
    constructable = constructableFunctionRegex.test(fnString) || classRegex.test(fnString); // 通过正则判断是否可构造
  }
...
export function getTargetValue(target: any, p: any): any {
  ...
  if (isCallable(value) && !isBoundedFunction(value) && !isConstructable(value)) {
    ...
    if (value.hasOwnProperty("prototype") && !boundValue.hasOwnProperty("prototype")) {
      ...
      Object.defineProperty(boundValue, "prototype", { value: value.prototype, enumerable: false, writable: true }); // 若原函数有 prototype，则为绑定函数补上 prototype，防止构造相关问题
    }
...
export function isMatchSyncQueryById(id: string): boolean {
  const queryMap = getAnchorElementQueryMap(anchorElementGenerator(window.location.href)); // 解析当前地址的查询参数
...
export function fixElementCtrSrcOrHref(
  ...
  elementCtr.prototype.setAttribute = function (name: string, value: string): void {
    ...
    if (name === attr) targetValue = getAbsolutePath(value, this.baseURI || "", true); // 若设置的是目标属性（src/href），转换为绝对路径
    rawElementSetAttribute.call(this, name, targetValue); // 调用原始 setAttribute 设置属性
  };
  // patch href get and set
  const rawAnchorElementHrefDescriptor = Object.getOwnPropertyDescriptor(elementCtr.prototype, attr); // 获取原有属性描述符（get/set）
...
// @ts-ignore
export const requestIdleCallback = window.requestIdleCallback || ((cb: Function) => setTimeout(cb, 1)); // 在不支持 requestIdleCallback 的环境下回退为 setTimeout

export function getContainer(container: string | HTMLElement): HTMLElement {
  return typeof container === "string" ? (document.querySelector(container) as HTMLElement) : container; // 支持传入选择器字符串或元素本身，返回 HTMLElement
}
...
export function compose(fnList: Array<Function>): (...args: Array<string>) => string {
  return function (code: string, ...args: Array<any>) {
    return fnList.reduce((newCode, fn) => (isFunction(fn) ? fn(newCode, ...args) : newCode), code || ""); // 按顺序将 code 通过函数链处理并返回最终结果
  };
}
...
export function mergeOptions(options: cacheOptions, cacheOptions: cacheOptions) {
  return {
    ...
    // 默认 {}
    attrs: options.attrs !== undefined ? options.attrs : cacheOptions?.attrs || {}, // attrs 若未定义则 fallback 到缓存或空对象
    degradeAttrs: options.degradeAttrs !== undefined ? options.degradeAttrs : cacheOptions?.degradeAttrs || {}, // degradeAttrs 合并策略同上
    // 默认 true
    fiber: options.fiber !== undefined ? options.fiber : cacheOptions?.fiber !== undefined ? cacheOptions?.fiber : true, // fiber 默认为 true，优先使用 options 的值
    alive: options.alive !== undefined ? options.alive : cacheOptions?.alive, // alive 优先 options
    degrade: options.degrade !== undefined ? options.degrade : cacheOptions?.degrade, // degrade 优先 options
    plugins: options.plugins || cacheOptions?.plugins, // plugins 优先 options
    iframeAddEventListeners: options.iframeAddEventListeners || cacheOptions?.iframeAddEventListeners || [], // iframeAddEventListeners 合并并默认空数组
    iframeOnEvents: options.iframeOnEvents || cacheOptions?.iframeOnEvents || [], // iframeOnEvents 合并并默认空数组
    lifecycles: {
